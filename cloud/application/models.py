...
class Post(db.Model):
    ...
    photos = db.Column(db.String(128),unique=True, index=True)
    timestamp = db.Column(db.DateTime, index=True, default=datetime.utcnow)
    author_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    comments = db.relationship('Comment', backref='post', lazy='dynamic')
    photos_t = db.Column(db.String(128),unique=True, index=True)
    @property
    def approve_count(self):
        return User_Approve.query.filter(User_Approve.target_id == self.id).count()
    @staticmethod
    def create_fake(count=100):
        """
        :summary: 创建虚拟的文章
